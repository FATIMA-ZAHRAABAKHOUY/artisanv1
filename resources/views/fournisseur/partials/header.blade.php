...
<div style="background:var(--indigo);padding:14px 20px;display:flex;
  ...
  <div style="display:flex;align-items:center;gap:10px;">
    <div style="width:36px;height:36px;border-radius:50%;
      ...
      @if(auth()->user()->fournisseur?->getLogoUrl())
        <img src="{{ auth()->user()->fournisseur->getLogoUrl() }}" alt="" style="width:100%;height:100%;object-fit:cover;">
      @elseif(auth()->user()->avatar)
        <img src="{{ asset('storage/'.auth()->user()->avatar) }}" alt="" style="width:100%;height:100%;object-fit:cover;">
      @else
        {{ strtoupper(substr(auth()->user()->fournisseur?->nom ?? auth()->user()->prenom ?? 'F', 0, 1)) }}
      @endif
    </div>
    <div>
      <div style="color:white;font-size:14px;font-weight:600;">
        {{ auth()->user()->fournisseur?->nom ?? auth()->user()->nom_complet }}
      </div>
  ...
  <div style="display:flex;gap:8px;flex-wrap:wrap;">
    ...
    <a href="{{ route('fournisseur.produits') }}"
       style="padding:7px 14px;font-size:13px;color:white;
              background:rgba(255,255,255,0.08);border-radius:var(--radius-sm);
              text-decoration:none;display:flex;align-items:center;gap:6px;">
